...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/adambensHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary=search_gifs, backref=db.backref('search', lazy='dynamic'), lazy='dynamic')
...
def get_gifs_from_giphy(search_string):
    ...
    g_list = json.loads(response.text)['data']
    return g_list
...
def get_or_create_gif(title, url):
    ...
    gif_instance = Gif.query.filter_by(title=title).first()
    if gif_instance:
        return gif_instance
    else:
        gif_instance = Gif(title=title, embedURL=url)
        db.session.add(gif_instance)
        db.session.commit()
        return gif_instance
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        gif_search = get_gifs_from_giphy(term)
        for gif in gif_search:
            title = gif['title']
            url = gif['embed_url']
            gif_obj = get_or_create_gif(title, url)
            search_term.gifs.append(gif_obj)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    personal_collection = PersonalGifCollection.query.filter_by(name=name, user_id=current_user.id).first()
    if personal_collection:
        return personal_collection
    else:
        personal_collection = PersonalGifCollection(name=name, user_id=current_user.id, gifs=gif_list)
        for gif in gif_list:
            personal_collection.gifs.append(gif)
...
def create_collection():
    ...
    if request.method == 'POST':
        ...
        gif_obj = [get_gif_by_id(int(id)) for id in gifs_selected]
